...
SubPolygon::SubPolygon(VECTOR2D *aVertices, int aVertexCount)
{
    ...
    mVertexData = new VertexData[NUM_OF_POLYGON_VERTEX];
	m_nVertextDataAlloc = NUM_OF_POLYGON_VERTEX;
}
...
bool SubPolygon::init_poly(VECTOR2D *aVertices, int aVertexCount)
{ 		
	mVertices = aVertices;
	if (mVertices == NULL)
		return false;
	if (aVertexCount+1 > m_nVertextDataAlloc)	
	{
		if (mVertexData)
			delete [] mVertexData;
		mVertexData = new VertexData[aVertexCount+1];
		m_nVertextDataAlloc = aVertexCount+1;
	}	
    if (mVertexData == NULL)
	{
		mVertexCount = 0;
 ...
 */
int SubPolygon::getScanEdges(PolygonScanEdge *aEdges, const ClipRectangle &aClipRectangle) const
{
    int n;
    for (n = 0; n < mVertexCount; n++)
    {
        // Transform all vertices.
		mVertexData[n].mPosition.mX = mVertices[n].mX;
		mVertexData[n].mPosition.mY = mVertices[n].mY * SUBPIXEL_COUNT;        
    }
    ...
    for (n = 0; n < mVertexCount; n++)
    {
        ...
        else
        {
			...
			if(n > 0 && ((mVertexData[n].mLine - mVertexData[n-1].mLine == 1) || (mVertexData[n].mLine - mVertexData[n-1].mLine == -1))) 
			{
				int startIndex, endIndex;      				
				if (VECTOR2D_GETY(mVertexData[n - 1].mPosition) < VECTOR2D_GETY(mVertexData[n].mPosition))
				{
					startIndex = n - 1;
					endIndex = n;                
				}
				...
				if(VECTOR2D_GETY(mVertexData[endIndex].mPosition) - VECTOR2D_GETY(mVertexData[startIndex].mPosition) < SUBPOLYGON_EPSILON)
				{					
					mVertexData[startIndex].mLine = mVertexData[endIndex].mLine;			
				}
			}
        }
    }	
